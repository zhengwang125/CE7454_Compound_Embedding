...
f_node_labels = open('BZR_node_labels.txt')
f_node_attributes = open('BZR_node_attributes.txt')
f_graph_indicator = open('BZR_graph_indicator.txt')
f_graph_labels = open('BZR_graph_labels.txt')
f_graph = open('BZR_A.txt')
...
def find_skeleton(ID):
    ...
    #nx.draw(G_collection[ID], with_labels = True, node_color='y', node_size = 200, font_size=10)
    #plt.show()
...
if __name__ == '__main__':
    ...
    G_collection, G_labeldict = build_graph(Bond_list, Graph_Indicator_list, total = len(Graph_Label_list))
